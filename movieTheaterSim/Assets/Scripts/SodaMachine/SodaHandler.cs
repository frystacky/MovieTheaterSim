...
public class SodaHandler : MonoBehaviour
{
    ...
    public float sodaFillLevel;
    public string sodaTypeName;
    public float sodaSizeMaxFillLevel; //small soda=50 Medium=100 and large=150
        ...
        if (Input.GetButtonDown("Interaction"))
        {
            ...
            if (objectGrabPointTransform != null)
            {
                ...
                if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out RaycastHit raycastHit, sodaPlaceDistance, layerMask))
                {
                    ...
                    {
                        //Debug.Log(hitItemObjectHandler.gameObject.name + " HIT!");
                        objectGrabbable.Drop(this.gameObject);
                        this.transform.position = hitItemObjectHandler.parent.transform.position;
